...
# FONCTION DE RECHERCHE NAIVE (TOUTES LES OCCURENCES)
def rech_naive2(T,M):
    ...
    if n>=m : # verification si la taille du texte T est bien grande que celle du motif M
        ...
        list_pos = [] # creation de la liste qui conteindera les positions d'apparition du motif M dans le texte T
        while i<n-m+1: # Parcours du texte 
            ...
            if j==m: # verifier si le compteur j a atteint la taille du motif (motif trouve dans le texte a la position i)
                list_pos.append(i) # inserer la position dans la liste des positions
            i+=1 # incrementation du compteur de parcours du Texte pour passer au prochain caractere pour continuer la recherche d'autres occurences
        t2 = time.perf_counter() # affecter le temps de fin du calcul a la variable t2
        if len(list_pos) > 0: # verfier si la liste n'est pas vide
            return (list_pos, nbr_comparaison-1, t2-t1) # retourner le resultat dans la forme suivante (la liste des positions, nombre de comparaison, temps d'execution)
        else: # dans le cas ou la liste est vide
            list_pos.append(-1) # on insere -1 a la premiere case
            return (list_pos, nbr_comparaison-1, t2-t1) # retourner le resultat dans la forme suivante (liste contenant la valeur -1, nombre de comparaison, temps d'execution)
    else: # Dans le cas ou la taille de T est supp de celle de M
        list_pos.append(-2) # on insere -1 a la premiere case
        return (list_pos, 0, 0) # retourner le resultat dans la forme suivante (liste contenant la valeur -2, nombre de comparaison, temps d'execution)
...
def rech_MP_2(T, M):
    ...
    if n >= m :  # verification si la taille du texte T est bien grande que celle du motif M
        ...
        i = 0  # initialisation du compteur de parcours du texte a 0
        j = 0  # initialisation du compteur de parcours du motif a 0
        bord = tab_bords(M) # creation du tableau des meilleures bords du motif M en entree en utilisant la fonction definis auparavant
        nbr_comparaison = 0 # initialisation du compteur de nombre de comparaison a 0
        lst_mp = []  # creation de la liste qui conteindera les positions d'apparition du motif M dans le texte T
        while i<n-m+1 :  # Parcours du texte 
            while (j<m) and (T[i+j]==M[j]): # Parcours du motif tout en verifiant si T[i+j] est egale a M[j]
                j += 1 # incrementation de j pour passer au prochain caractere
                nbr_comparaison += 1 # incrementation du compteur de comparaison par 1
            if j==m: # verifier si le compteur j a atteint la taille du motif (motif trouve dans le texte a la position i)
                lst_mp.append(i) # inserer la position dans la liste des positions
        ...
        t2 = time.perf_counter() # affecter le temps de fin du calcul a la variable t2
        if len(lst_mp) > 0: # verfier si la liste n'est pas vide
            return (lst_mp, nbr_comparaison, t2-t1) # retourner le resultat dans la forme suivante (la liste des positions, nombre de comparaison, temps d'execution)
        else: # dans le cas ou la liste est vide
            lst_mp.append(-1) # on insere -1 a la premiere case
            return (lst_mp, nbr_comparaison, t2-t1) # retourner le resultat dans la forme suivante (liste contenant la valeur -1, nombre de comparaison, temps d'execution)
    else: # Dans le cas ou la taille de T est supp de celle de M
        lst_mp.append(-2) # on insere -1 a la premiere case
        return (-2, 0, 0)  # retourner le resultat dans la forme suivante (liste contenant la valeur -2, nombre de comparaison, temps d'execution)

#----------------------------------------------------------------------------------------------------------------------------------------
# FONCTION DU MEILLEUR BORD
def best_bord(M):
    bestbord = []
    bestbord.append(-1)
    i = -1
...
def rech_KMP(T, M):
    ...
    if n>=m : # verification si la taille du texte T est bien grande que celle du motif M
        ...
        nbr_comparaison = 0 # initialisation du compteur de nombre de comparaison a 0
        lst_kmp = [] # creation de la liste qui conteindera les positions d'apparition du motif M dans le texte T
        while i<n-m+1: # Parcours du texte 
            nbr_comparaison +=1 # incrementation du compteur de comparaison par 1
            while (j<m) and (T[i+j] == M[j]):  # Parcours du motif tout en verifiant si T[i+j] est egale a M[j]
                nbr_comparaison +=1 # incrementation du compteur de comparaison par 1
                j += 1 # incrementation de j pour passer au prochain caractere
            if (j == m): # verifier si le compteur j a atteint la taille du motif (motif trouve dans le texte a la position i)
                lst_kmp.append(i) # inserer la position dans la liste des positions
            ...
            if bestbord[j] > 0:  # pour ne pas tester les cararcteres de M deja testes, on verifie si le bord est supperieur a 0 (cas VRAI)
                ...
                j=0 # on remet la variable j a 0 
        t2 = time.perf_counter() # affecter le temps de fin du calcul a la variable t2
        if len(lst_kmp) > 0:  # verfier si la liste n'est pas vide
            return (lst_kmp, nbr_comparaison-1, t2-t1) # retourner le resultat dans la forme suivante (la liste des positions, nombre de comparaison, temps d'execution)
        else: # dans le cas ou la liste est vide
            lst_kmp.append(-1) # on insere -1 a la premiere case
            return (lst_kmp, nbr_comparaison-1, t2-t1) # retourner le resultat dans la forme suivante (liste contenant la valeur -1, nombre de comparaison, temps d'execution)
    else: # Dans le cas ou la taille de T est supp de celle de M
        lst_kmp.append(-2) # on insere -1 a la premiere case
        return (lst_kmp, 0, 0) # retourner le resultat dans la forme suivante (liste contenant la valeur -2, nombre de comparaison, temps d'execution)
...
def rech_RK(T, M):
    ...
    if n>m: # verification si la taille du texte T est bien grande que celle du motif M
        t1 = time.perf_counter() # initialisation du compteur de temps d'execution
        alphabet = { # creation du dictionnaire 'alphabet' contenant les alphabets du texte et leur poids
        ...
        nbr_comparaison = 0 # initialisation du compteur de nombre de comparaison a 0
        lst_pos = [] # creation de la liste qui conteindera les positions d'apparition du motif M dans le texte T
        ...
        for i in range(n-m+1): # parcours du texte
            if hash_T == hash_M: # verifier si le hash code de la chaine extraites du texte qui commence a la position i est egale au hash code du motif (cas TRUE)
                if equal_str(M, T[i:i+m]): # verifier si la chaine est egale au motif ( car les erreurs au calcul du hash code sont possibles) en utilisant la fonction equal_str defini un peu plus haut dans le code
                    ...
                    lst_pos.append(i) # inserer la position dans la liste des positions
            ...
            if i < n-m:  # verifie si le nombre de caracteres restants est egale a la taille du motif M
                ...
                break # on sort de la boucle qui parcourt le texte
        t2 = time.perf_counter() # affecter le temps de fin du calcul a la variable t2
        if len(lst_pos)>0: # verfier si la liste n'est pas vide
            return(lst_pos, nbr_comparaison, t2-t1) # retourner le resultat dans la forme suivante (la liste des positions, nombre de comparaison, temps d'execution)
        else: # dans le cas ou la liste est vide
            lst_pos.append(-1) # on insere -1 a la premiere case
            return(lst_pos, nbr_comparaison, t2-t1) # retourner le resultat dans la forme suivante (liste contenant la valeur -1, nombre de comparaison, temps d'execution)
    else: # Dans le cas ou la taille de T est supp de celle de M
        lst_pos.append(-2) # on insere -1 a la premiere case
        return (lst_pos, 0, 0) # retourner le resultat dans la forme suivante (liste contenant la valeur -2, nombre de comparaison, temps d'execution)
